...
const buttonCss = "hover:bg-power-purple transition duration-200 rounded-xl hover:cursor-pointer text-lg text-white/90 h-10 flex items-center px-3 gap-2 py-2 ";
        ...
        <div className="h-full w-3/20 bg-slate-700 rounded-2xl flex flex-col p-4">
            ...
            <div className="flex flex-col mt-4 gap-2">
                {buttons.map((button, index) => {
                    return (
                        <button className={buttonCss +(button.isActive ? "bg-power-purple" : "")} key={index}>
                            {button.icon}
                            {button.name}
                        </button>
                    )})}
            ...
            <div className="w-full h-24 pt-2 border-t border-white/40 mt-auto flex flex-col gap-2">
                <button className={buttonCss + " w-full"}>
                    Help
                </button>
                <button className={buttonCss + " w-full"}>
                    LogOut
